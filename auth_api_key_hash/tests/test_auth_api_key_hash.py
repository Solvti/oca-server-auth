...
class TestAuthApiKey(SavepointCase):
    ...
    def setUpClass(cls, *args, **kwargs):
        ...
        cls.AuthApiKey = cls.env["auth.api.key.hash"]
    ...
    def test_lookup_key_from_env(self):
        self.assertEqual(
            self.env["auth.api.key.hash"]._retrieve_uid_from_api_key(
                "api_key_from_env"
            ),
            self.demo_user.id,
        )
        with self.assertRaises(ValidationError):
            ...
            self.env["auth.api.key.hash"]._retrieve_uid_from_api_key("dummy")
    ...
    def test_cache_invalidation(self):
        ...
        self.assertEqual(
            self.env["auth.api.key.hash"]._retrieve_uid_from_api_key("updated_key"),
            self.demo_user.id,
        )
        with self.assertRaises(ValidationError):
            self.env["auth.api.key.hash"]._retrieve_uid_from_api_key("api_key")
